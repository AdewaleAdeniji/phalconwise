...
function getTrxs(){
    http('/transactions/transactions')
    .then(response=>response.json())
    .then((data)=>{
        if(response(data)){
            ...
            if(count==0){
                ...
                transactions.content.forEach((trx)=>{
                    ...
                    var trxhtml = `<div class="notification__item" data-trx=${trx.transactionid}>
                    <div class="notification__ava">
                      
                    </div>
                    <div class="notification__details">
                      <div class="notification__author title">${trx.type=="0" ? 'Transaction' : trx.type }</div>
                      <div class="notification__line caption">
                        <div class="notification__category">${trx.transaction}</div>
                        <br/>
                        </div>
                        <div class="notification__time notification__info">&#8358;${trx.amount}</div>
                      <div class="notification__text">${moment(moment(trx.created_at).format('lll')).add(1, 'hours').calendar()}</div>
                      <div class="actions">
                          <button class="actions__btn reportbtn" data-trx=${trx.transactionid} title="Report/Contact customer care concerning this transaction">
                      ...
                      <div class="notification__preview bg-purple-light">
                          <img class="notification__pic" src="img/figure-1.png" alt="" />
                          </div>
                  </div>`;
                ...
                get('trxs').innerHTML=html;
            }
        }
    })
    .catch((err)=>{
        console.log(err);
        get('trxs').innerHTML='Transactions Fetch Failed <a href="#" onclick=getTransactions()>Retry</a>';
    })
}
