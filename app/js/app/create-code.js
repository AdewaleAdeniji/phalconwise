...
$('.createcode').submit(function(e){
    e.preventDefault();
    
    var codename = value('codename');
    var codeamount = value('codeamount');
    var codeinterval = value('codeinterval');
    var codenote = value('codenote');
    if(codename==""){
        inform('Code name cannot be empty');
    }
    else if(codeamount<50){
        inform('Code amount cannot be less than &#8358;100');
    }
    else if(codeinterval<1){
        inform('Code interval must be one or more')
    }
    else {
        var json = {
            "amount":codeamount,
            ...
            "codepin":"",
            "codenote":codenote,
            "codeseekapproval":false,
            "codeshowname":true
        };
        var payload = JSON.stringify(json);
        load('Creating Code ...');
        httpPost('/code/create',payload)
        .then(response=>response.json())
        .then((data)=>{
            closeload();
            if(response(data)){
        ...
        .catch((err)=>{
            closeload();
            inform('Error Occurred while processing your request');
        })
    }
