...
$('.sendmoney').submit(function(e){
    ...
    if(amount<10){
        inform('Amount must be greater than 10 naira');
    }
    else if(username==""||username.length<4){
        inform('Username must have four or more characters');
    }
    else {
        const data = JSON.stringify({"username":username,"amount":amount});
        load('Sending Money');
        httpPost('/transfer/send',data)
        .then(response=>response.json())
        .then((data)=>{
            closeload();
            if(data.code==200){
